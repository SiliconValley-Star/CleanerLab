...
const App = ()=>{
    ...
    const [total,setTotal]=useState(null)// burda gettotaldiskcapacityi alıyor ve total değişkenine tanımlıyor ilk olarak null olarak gönderiyor sonrasında settotal ile burda capacityi çekiyor
    const [totalByte,setTotalByte]=useState(null)
    const [last,setLast]=useState(false)// burda ilk uygulama açılırken introsunu check ediyor eğer setlast true dönüyorsa artık intro dönmüyor.

    const [free,setFree]=useState(null)//burda getfreediskstorage kullanıcının cihazındaki boş alanı hesaplıyoruz değişken olarak frre,setfree tanımlıyoruz.
    const [percent,setPercent]=useState(null) //yüzde kaç dolu olduğunu burdan hesaplıyoruz
    const [count,setCount]=useState(0)

    //format olarak bize byte olarak geliyor bunu burada mb,gb gibi formatlara çeviriyoruz
    ...
    //cihazın yüzde kaç dolu olduğunu gösteren fonksiyon
    ...
    let notify = ()=>{
        ...
        OneSignal.setAppId("fd02c62d-7dba-4309-aedb-851adfe7059b"); //burda onsignalapp key kullanıyoruz

        //burda kullanıcıya soruyoruz uygulama kullanıcıya bildrim göndersinmi diye bu cevaba göre kullanıcıyı kaydediyoruz ve bildirim atabiliyoruz.
        ...
        //burda kullanıcıya yukardan bildirim geldiğinde o bildirime göre tıklarsa o sayfaya yönlendiriyoruz.
    ...
    let getBytes = ()=>{
        // burda gettotaldiskcapacityi alıyor ve total değişkenine tanımlıyor ilk olarak null olarak gönderiyor sonrasında settotal ile burda capacityi çekiyor
        getTotalDiskCapacity().then(data=>{
            ...
            //burda getfreediskstorage kullanıcının cihazındaki boş alanı hesaplıyoruz değişken olarak frre,setfree tanımlıyoruz.
            getFreeDiskStorage().then(value=>{
                if(Platform.OS=='android'){
                    setFree(formatBytes(exact-value).split(' ')[0])
                    // alert(data)
                    // alert(exact)
                    //setpercent ile yüzde kaç dolu olduğunu hesaplıyoruz.
    ...
    useEffect(()=>{
        ...
        //kullanıcının kullandığı numaranın ülke kodunu alıyoruz
    ...
    //intro girişi uygulamaya ilk girdiğinde next next yaptığımız kısım
    const slides = [
        {
            key: 1,
            title: I18n.t('title1'),
    ...
    //burda intro kısmını döndürmek için appintroslider kısmına gidip orada index 0,1,2 döndürüyor ve kullanıcıya sunuyor renderiteme tıkla appintroya git
                ...
                <Container style={styles.slide}>
                    ...
                    <View style={[styles.header,{flexDirection:'row',height:'10%'}]}>
                        <View style={{alignItems:'center'}}>
                            ...
                            <Text>{I18n.t('photos')}</Text>{/* burda dil paketini çalıştırıyor lang dosyasından photosu bulup laks diye yapıştırıyor hangi dil ile kullanılıyorsa */}
                        </View>
    ...
    //startshake butonu sallıyor animasyon ekliyoruz.
    let startShake = () => {
        Animated.sequence([
  ...
  let checkPhotos=()=>{
    if(Platform.OS == 'android'){
        ...
        //burda önce kullanıcındn izin istiyor permission kütüphanesi ile
  ...
  useEffect(()=>{
    ...
    const screen = Dimensions.get('window') // react-nativenin kendi paketi dimensions otomatik olarak width height alıyor.

    if(userStore.loading ){ //burda dinamik olarak  loading true ise açılış videosunu döndürüyor openin.mp4
        return(
    ...
    }else if(userStore.first){ //userstore.first true ise kullanıcıya introda slideslar gösteriliyor
        return(
            // burda ilk uygulama açılırken introsunu check ediyor eğer setlast true dönüyorsa artık intro dönmüyor.
            <AppIntroSlider
                data={slides}
        ...
        {/* eğer herşey tamamsa uygulamayı başlatır */}
        return(
            <View style={{flex:1}}>
                <KeepAwake /> {/* keepawake bir paket ekranın sürekli açık kalmasını sağlar */}
