...
import DeviceInfo from 'react-native-device-info'; //device'ın kimlik adresini almak icin imei icin kullaniyoruz 
import AsyncStorage from "@react-native-community/async-storage"; //mpbul cihaz icinde fiziksel bir depolama alani var local storage 
import  CameraRoll  from "@react-native-community/cameraroll"; //fotoğrafları import etmek icin kullaniyoruz
...
import {AppEventsLogger} from "react-native-fbsdk-next"; //facebook paketi kullanılıyor burda uygulama girisi cikisi takip ediliyor
...
class userStore {
    first=false //bununla uygulama başlıyor yani intro döndürmüyor artık direk uygulamaya geçiş yapıyor
    token = null
    checkedImages=[]
    deleteLoading=false
    photos = []
    ...
    constructor(username) {
        //observable ile üstteki değişkenleri dinamik hale getirdik
        makeObservable(this, {
            adActive:observable,
    ...
    firststep(){
        ...
        // AsyncStorage.clear()
        this.check() //this.check fonksiyonu gidip token varmı onu kontrol ediyor. eğer yoksa else kısmında kullanıcıyı baştan kaydediyor.
    ...
    //ilk baştaki uygulama açılış videosu açık geliyor bu fonksiyon kapatmayı sağlıyor
    startApp(){
        runInAction(()=>{
            setTimeout(()=>{
                this.loading=false
    ...
    //Eğer Premium değilse kullanıcının limitleri var
    getTodaysLimit(){
        // dailyOther
    ...
    //getDeleteAfter ayarlar kısmında Remove After Import switchine atılan bir değişken
    getDeleteAfter(){
    ...
    //premium packageları çekiyor ios ve androide göre ayarlıyor
    setReadyPackages(){
    ...
    startToCheck(){
        //eğer finished false ise fonksiyona giriyor ve başlıyor lastindex photoOptimized'e eşit değilse 5sn sonra tekrardan startToCheck() fonksiyonuna giricek eğer eşitse comparingStarted'ı false olarak alıp 1sn sonra comparePhotos'u çalıştırıcak
        if(!this.finished){
            if(this.lastIndex != 0){
    ...
    //fotoğraflar optimize edildi mi onu kontrol ediyoruz
    checkToCompare(){
    ...
    async compressAllImages() {
        try {
            ...
            // Önceki sıkıştırma adımının kaydedilip kaydedilmediğini kontrol et(Burda lastChunk var)
            AsyncStorage.getItem('compressChunk').then(lastChunk => {
                if (lastChunk) { // Eğer bir önceki adım varsa devam et
                    ...
                    // lastchunk yoksa sadece i'ye atanan değer değişiyor geri kalan işlemler yukarısı ile aynı İlk adımda
                    for (let i = 0; i < this.checkedImages.length; i += chunkSize) {
                        let chunk = this.checkedImages.slice(i, i + chunkSize);
                        setTimeout(() => {
                            this.checkedImages.slice(i, i + chunkSize).find((it, ind) => {
    ...
    getAllPhotos(){
        ...
        AsyncStorage.getItem('wholeImages').then(dat=>{
            if(dat){
                ...
                //burası anasayfada others kısmındaki fotoğraflara hizmet ediyor tek seferde hepsini çekiyor.
            ...
            MediaLibrary.getAssetsAsync({first:15,after:last,sortBy:['creationTime']})
                .then(async res => {
                    ...
                    res.assets.forEach((i,index)=>{
                        ...
                        // 15tanesini çağırıp test ediyor gibi düşün. burda creationTime'a aldığımız checkImages kısmını tekrar çekip boyutlandırıp resizedImage kısmına yolunu ekliyor.
                        ImageResizer.createResizedImage(this.changeLink(i.uri),16,16,'JPEG',1)
                            .then(son=>{
    ...
    //kullanıcının bildirim playerId'sini almak için kullandığımız fonksiyon.
    getPlayerId=()=>{
    ...
    //kullanıcının playerId'sini güncellemek için
    setPlayerId=(id)=>{
        runInAction(()=>{
            this.playerId=id
        })
    ...
    //kullanıcının playerId'sini veritabanında güncellemek için
    ...
    //Kullanıcının verilerini güncellemek için yaptığımız her değişiklik buna dahil
    updateInfo(){
        DeviceCountry.getCountryCode(TYPE_ANY)
            .then((result) => {
    ...
    //getSimilar anasayfadaki SimilarPhotos kısmını getiriyor bunu arkada boyutlandırıp,kalitesini ve formatını ayarlayıp getiriyor.
    getSimilar(){
    ...
    //burası premium'u kontrol ediyor varmı yokmu
    premiumGate(){
        if(!this.premiumSync) this.changePremium()
    }
    //burası premium paketi değişikliği için 
    changePremium(){
        runInAction(()=>{
    ...
    //fotoğrafları secretPlace'e koymak için
    setPhotos(assets){
    ...
    // eventlistenerdan gönderilen msgler
    setSimilar(it){
...
// ...
    ...
    //burda uygulamada olan secretplace'e verilen şifreyi kontrol ediyor 
    getPassword(){
    ...
    //burası secretplace içerisinde tutulan fotoğrafları getiriyor
    getKept(){
    ...
    getFirst(){
        //önce key varmı diye kontrol ediyoruz eğer varsa devam ediyor eğer yoksa introyu tekrar döndürüyoruz.
